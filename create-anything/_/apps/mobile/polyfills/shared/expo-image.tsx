...
// Export our wrapped Image component - assign static properties after creation
const Image = WrappedImage;

// Copy static properties from ExpoImage.Image to our Image component
// Do this in a way that avoids circular dependency
setTimeout(() => {
  try {
    const originalImage = ExpoImage.Image as any;
    if (originalImage) {
      const staticKeys = Object.getOwnPropertyNames(originalImage).filter(
        (key) => key !== 'prototype' && key !== 'name' && key !== 'length'
      );
      for (const key of staticKeys) {
        try {
          const descriptor = Object.getOwnPropertyDescriptor(originalImage, key);
          if (descriptor && !(key in Image)) {
    ...
    // Ignore errors - static properties are optional
  }
}, 0);

// Re-export types from expo-image
export type { ImageProps, ImageSource, ImageErrorEventData } from 'expo-image';

// Export our wrapped Image
export { Image };
